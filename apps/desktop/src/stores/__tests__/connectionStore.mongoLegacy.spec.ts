...
describe("connectionStore MongoDB Legacy fallback", () => {
  ...
  it("uses the persisted Web fallback profile to hide native-only actions", async () => {
    ...
    store.connections[0] = {
      ...store.connections[0],
      name: "MongoDB updated while connecting",
      read_only: true,
    };
    ...
    expect(store.getConfig(config.id)?.name).toBe("MongoDB updated while connecting");
    expect(store.getConfig(config.id)?.read_only).toBe(true);
  });
});
