...
CSV follows MongoDB Compass: a header row, nested fields as dotted paths (`address.city`), ObjectId as 24-character hex, dates as ISO-8601, and empty cells as null. Import infers column types the same way Compass does; `_id` hex is restored as ObjectId. JSON/NDJSON still use Extended JSON (`$oid`, `$date`). All strings keeps every cell as text.

First-release limits: append only (no truncate or upsert), no Excel/GridFS/BSON binary import, and no automatic retry of failed batches.
...
- The export always continues through the complete query result. It does not stop at the current page.
- CSV nested objects and arrays are Extended JSON strings. NDJSON writes one canonical Extended JSON document per line.
