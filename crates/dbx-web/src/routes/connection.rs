...
async fn apply_mongo_legacy_driver_profile(state: &WebState, config: &ConnectionConfig) -> Result<(), AppError> {
    ...
    mark_mongo_legacy_driver(&mut runtime_config);
    ...
    if !config.one_time {
        state
            ...
            .save_connection_driver_profile(
                ...
                runtime_config.driver_profile.clone(),
                runtime_config.driver_label.clone(),
            )
            .await
            .map_err(AppError::from)?;
    }
    let mut runtime_configs = state.app.configs.write().await;
    if let Some(current) = runtime_configs.get_mut(&config.id) {
...
mod tests {
    ...
    async fn mongo_legacy_profile_sync_preserves_unrelated_saved_connections() {
        ...
        current.name = "Mongo updated while connecting".to_string();
        ...
        assert_eq!(runtime.name, "Mongo updated while connecting");
