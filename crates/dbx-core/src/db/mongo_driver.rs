...
pub async fn create_index(
    ...
    client.database(database).run_command(command).await.map_err(|e| e.to_string())?;
    Ok(name)
}
...
fn parse_create_index_spec(
    ...
    // The sidebar form accepts JSON rather than a Mongo shell expression.
...
mod tests {
    ...
    fn create_indexes_command_keeps_raw_options_and_generates_a_name() {
        ...
        assert_eq!(index.get_bool("unique").unwrap(), true);
